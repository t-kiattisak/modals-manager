import {
  ...
  OpenConfirmModal,
  OpenContextModal,
} from "../models/modals-context.model"
import { createUseExternalEvents } from "./create-use-external-events.utility"
...
type ModalsEvents = {
  ...
  openContextModal: <TKey extends TModal>(
    payload: OpenContextModal<Parameters<TModals[TKey]>[0]["innerProps"]> & {
      modal: TKey
    },
  ) => string
  closeModal: (id: string) => void
  closeContextModal: <TKey extends TModal>(id: TKey) => void
  closeAllModals: () => void
  updateModal: (
    payload: { modalId: string } & Partial<ModalSettings> &
      Partial<OpenConfirmModal>,
  ) => void
  updateContextModal: (
...
export const updateContextModal = (
  payload: { modalId: string } & Partial<OpenContextModal<any>>,
